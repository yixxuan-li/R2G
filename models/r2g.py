...
class R2G(nn.Module):
    ...
    def __call__(self, batch: dict) -> dict:
        ...
        # node_attr = torch.cat([object_semantic_prob.unsqueeze(2), color_semantic_prob.unsqueeze(2), function_semantic_prob.unsqueeze(2), \
        #                        ls_attr.unsqueeze(2), tl_attr.unsqueeze(2), mc_attr.unsqueeze(2), tb_attr.unsqueeze(2), lr_attr.unsqueeze(2), \
        #                         losh_attr.unsqueeze(2), curve_attr.unsqueeze(2)], 2) # B X N X embedding -> B X N X L+1 X embedding, (B * 52 * 2 * 300) 
        ...
        if self.args.relation_pred:
            ...
            edge_attr = edge_prob_logits.cuda().float() @ concept_vocab[self.concept_vocab_seg[-2]:]
            ...
            edge_attr = batch['edge_attr'].cuda().float() @ concept_vocab[self.concept_vocab_seg[-2]:]
        ...
        if not self.args.language_relation_alpha > 0:
            ...
            final_node_distribution, encoded_questions, anchor_logits, relation_logits, target_logits, simi, simi_index = self.nsm(node_attr = node_attr, edge_attr = edge_attr, description = language_embedding, property_embeddings = property_embedding, node_mask = batch['object_mask'].cuda(), context_size = batch['context_size'].cuda(), lang_mask = batch['lang_mask'].cuda().float(), language_len = batch['tokens_len'],  concept_vocab_seg = self.concept_vocab_seg, concept_vocab = concept_vocab, relation_vocab = concept_vocab[self.concept_vocab_seg[-2]:])
        else:
            final_node_distribution, encoded_questions, anchor_logits, relation_logits, target_logits, simi, simi_index = self.nsm(node_attr = node_attr, edge_attr = edge_attr, description = language_embedding, property_embeddings = property_embedding, node_mask = batch['object_mask'].cuda(), context_size = batch['context_size'].cuda(), lang_mask = batch['lang_mask'].cuda().float(), language_len = batch['tokens_len'],  concept_vocab_seg = self.concept_vocab_seg, concept_vocab = concept_vocab, relation_vocab = concept_vocab[self.concept_vocab_seg[-2]:])
