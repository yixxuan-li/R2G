...
class ScannetScan(object):
    ...
    def sample_scene_pc(self, n_samples):
        xyz = self.pc
        color = self.color
        ...
        return np.concatenate([s_xyz, s_color], axis=1)
    ...
    def visualize_ground(self, pid = None, id = None, filename = None):
        ...
        # get bbox
        points = self.pc
        color = self.color *256
        for _object in self.three_d_objects:
            if pid == _object.object_id:
                verts, colors_, indices = _object.write_bbox(1)
                colors = np.array([[255, 0, 0]]).repeat(np.shape(verts)[0], axis = 0)
                points = np.vstack((points, verts))
                color = np.vstack((color, colors))
            if id == _object.object_id:
                verts, colors, indices = _object.write_bbox(0)
                colors = np.array([[0, 255, 0]]).repeat(np.shape(verts)[0], axis = 0)
                points = np.vstack((points, verts))
                color = np.vstack((color, colors))
        write_ply_rgb(points, color, filename)

    def visualize_heatmap(self, prob= None, pid = None, pos_id=None, id = None, filedir = None, utterance = None, top_id = None):
        def write_ply_rgb(points, colors, filename, text=True, num_classes=None):
            """ Color (N,3) points with RGB colors (N,3) within range [0,255] as OBJ file """
            colors = colors.astype(int)
            points = [(points[i,0], points[i,1], points[i,2], colors[i,0], colors[i,1], colors[i,2]) for i in range(points.shape[0])]
        ...
        if osp.exists(filedir):
            ...
            # write_ply_rgb(points, color, osp.join(filedir, 'pred_class' + utterance + '.ply'))
