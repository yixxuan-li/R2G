...
from ..models import SR_Retrieval
...
if __name__ == '__main__':
    ...
    for scan in all_scans:
        ...
        obj_position = np.array([o.get_center_position() for o in scan.three_d_objects])
        obj_position = np.expand_dims(obj_position, axis=0)
        context_size = len_object
        context_size = np.expand_dims(context_size, axis=0)
        object_mask = np.ones((1, context_size)) 

        edge_distance =  np.zeros((len_object, len_object, 1), dtype=np.float32)

        for i in range(len_object):
            for j in range(i+1, len_object):
                edge_distance[i][j] = np.sqrt(np.sum(np.square(obj_position[0][i] - obj_position[0][j])))
                edge_distance[j][i] = edge_distance[i][j]

        edge_distance = np.expand_dims(edge_distance, axis=0)
